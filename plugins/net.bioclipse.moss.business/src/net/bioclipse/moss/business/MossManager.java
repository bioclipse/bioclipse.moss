...
//	}
	...
	public int getMatom(MossProperties mp){
		int matom= Atoms.ELEMMASK;//nollst�lla if =never, don't match atom/aromaticity
	...
	public int getMbond(MossProperties mp){
		int mbond = Bonds.BONDMASK;//nollst�lla = never,
	...
	public int getMrgat(MossProperties mp){
		int mrgat = Atoms.ELEMMASK ; //nollst�lla =never, 
	...
	public int getMrgbd(MossProperties mp){
		int mrgbd = Bonds.BONDMASK;//nollst�lla = never Aromatic bonds
		if(mp.getAromatic().equals("upgrade")){
			mrgbd &= Bonds.UPGRADE;}
		else if(mp.getAromatic().equals("downgrade")){
